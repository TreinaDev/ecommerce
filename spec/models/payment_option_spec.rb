...
describe PaymentOption do
  describe ".all" do
    it "should get payment option through API" do
      response = double("response", status: 200, body: '[{"name" : "Cartão de Crédito", "installments" : "12", "installments_value" : "10"},
    ...
    it "should return empty array if API return error" do
      response = double("response", status: 500, body: '[{"error" : "API indisponivel"}]')
